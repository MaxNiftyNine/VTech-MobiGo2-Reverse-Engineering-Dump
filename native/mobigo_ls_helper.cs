...
{
    private const string DllName = "VTech2010USBDllU.dll";
    ...
    private static void ConfigureDllSearchPath()
    {
        ...
        string vendorDir = Path.GetFullPath(Path.Combine(baseDir, "..", "vendor"));
        if (!Directory.Exists(vendorDir))
        {
            throw new DirectoryNotFoundException("vendor directory not found: " + vendorDir);
        }
        string dllPath = Path.Combine(vendorDir, DllName);
        if (!File.Exists(dllPath))
        {
            throw new FileNotFoundException("vendor DLL not found", dllPath);
        }
        if (!SetDllDirectory(vendorDir))
        {
            throw new InvalidOperationException("SetDllDirectory failed for " + vendorDir);
        }
    }
